...
const lap = (number: number, time: number | null, position: number | null = number): LapRecord => ({
	lap: number, lapTimeMs: time, sectorsMs: [null, null, null], position, gapToLeaderMs: null,
	compound: null, tyreAge: null, pitted: false, utc: "2026-06-11T12:00:00Z",
});
...
describe("analysis series", () => {
	it("clips pace outliers and omits drivers without timed laps", () => {
		const result = lapsToPaceSeries({ "1": [lap(1, 80_000), lap(2, 80_100), lap(3, 100_000)], "2": [] }, ["1", "2"], drivers);
